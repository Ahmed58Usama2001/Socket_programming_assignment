//student_name: Ahmed Usama Saad Abo Elkhier
...
int main() {
	 ...
	 if (bind(socket_listen,bind_address->ai_addr, bind_address->ai_addrlen)) //find the missing parameter name
	 {
	 ...
	 printf("Listening...\n");
	 if (listen(socket_listen, 10) < 0) //find the missing function and parameter names
	 {
	 ...
	 char request[1024];
	 int bytes_received = recv(socket_client, request, 1024, 0); //use recv() function to receive the incoming packet and save it in the *request* variable defined
	 ...
	 int bytes_sent = send(socket_client, response, strlen(response), 0); //use send() function to send the *response* back to the client
